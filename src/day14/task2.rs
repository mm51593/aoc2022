...
pub fn run() {
    ...
    let limit = input.iter()
        ...
        .reduce(|acc, elem| std::cmp::max(acc, elem)).unwrap() + 1;

    let mut count = 0;
    while map.get(&SOURCE).is_none() {
        drop_sand(&mut map, SOURCE, limit);
        count += 1;
    }
...
fn drop_sand(map: &mut HashMap<(i32, i32), BlockType>, position: (i32, i32), limit: i32) {
    if position.1 == limit { 
